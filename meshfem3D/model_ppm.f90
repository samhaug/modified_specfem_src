...
!
! Modified by Ross Maguire at the University of Michigan, Sep. 2016
! - this version reads dVs, dVp, and dRho from 'DATA/PPM/model.txt'
!
! for generic models given as depth profiles at lon/lat using a text-file format like:
!
! #lon(deg), lat(deg), depth(km), dVs(%), dVp(%), drho(%)
!  -10.00000  31.00000  40.00000  -1.775005  -1.500000  -0.324099
!  -10.00000  32.00000  40.00000  -1.056823  -0.493983  -0.291009
! ...
! NOTE: depth must be increasing 
!
!--------------------------------------------------------------------------------------------------
  ...
  ! file
  character(len=*),parameter :: PPM_path = "./DATA/PPM/model.txt"
  ...
  double precision,dimension(:),allocatable :: PPM_dvs,PPM_dvp,PPM_drho,PPM_lat,PPM_lon,PPM_depth
...
! standard routine to setup model
  ...
  if (myrank /= 0) then
    allocate(PPM_lat(PPM_num_v),PPM_lon(PPM_num_v),PPM_depth(PPM_num_v),PPM_dvs(PPM_num_v), &
             PPM_dvp(PPM_num_v),PPM_drho(PPM_num_v))
  endif

  call bcast_all_dp(PPM_dvs(1:PPM_num_v),PPM_num_v)
  call bcast_all_dp(PPM_dvp(1:PPM_num_v),PPM_num_v)
  call bcast_all_dp(PPM_drho(1:PPM_num_v),PPM_num_v)
...
!
  ...
  double precision ::    lon,lat,depth,dvs,dvp,drho
  character(len=256) ::  line

  !e.g. Mediterranean model
  ! counts entries
  counter = 0
  open(unit=IIN,file=trim(PPM_path),status='old',action='read',iostat=ier)
  if (ier /= 0) then
    write(IMAIN,*) ' error opening: ',trim(PPM_path)
    call flush_IMAIN()
    ! stop
  ...
  ier = 0
  do while (ier == 0 )
    read(IIN,*,iostat=ier) lon,lat,depth,dvs,dvp,drho
    if (ier == 0) then
      counter = counter + 1
    endif
  enddo
  ...
  if (counter < 1) then
    write(IMAIN,*)
    write(IMAIN,*) '  model PPM:',PPM_path
    ...
    write(IMAIN,*)
    write(IMAIN,*) 'model PPM:',trim(PPM_path)
  ...
  allocate(PPM_lat(counter),PPM_lon(counter),PPM_depth(counter),PPM_dvs(counter),&
           PPM_dvp(counter),PPM_drho(counter))
  PPM_min_dvs = 0.0
  PPM_max_dvs = 0.0
  PPM_dvs(:) = 0.0
  PPM_dvp(:) = 0.0
  ...
  do while (ier == 0 )
    ...
    if (ier == 0) then
      ...
      PPM_dvp(counter) = dvp/100.0
      PPM_drho(counter) = drho/100.0
...
! returns dvs,dvp and drho for given radius,theta,phi  location
  ...
  if (.not. GAUSS_SMOOTHING) then
    call get_PPMdvs_value(lat,lon,r_depth,dvs)
    call get_PPMdvp_value(lat,lon,r_depth,dvp)
    call get_PPMdrho_value(lat,lon,r_depth,drho)
    return
  endif
  ...
  do i=-NUM_GAUSSPOINTS,NUM_GAUSSPOINTS
    ...
    do j=-NUM_GAUSSPOINTS,NUM_GAUSSPOINTS
      ...
      do k=-NUM_GAUSSPOINTS,NUM_GAUSSPOINTS
        ...
        call get_PPMdvs_value(g_lat,g_lon,g_depth,g_dvs)
...
!
  ...
  subroutine get_PPMdvs_value(lat,lon,depth,dvs)
...
!
  ...
  subroutine get_PPMdvp_value(lat,lon,depth,dvp)
  ...
  double precision lat,lon,depth,dvp
...
!
  ...
  subroutine get_PPMdrho_value(lat,lon,depth,drho)
  ...
  double precision lat,lon,depth,drho
